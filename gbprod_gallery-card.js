...
window.Webflow.push(function () {
  ...
  items.forEach(function (item) {
    ...
    const button = item.querySelector('.btn');  // or '.card-button-wrapper'
...
const tl = gsap.timeline({
  paused: true,
  reversed: true,
  defaults: { duration: 0.3, ease: 'power2.out' } // Slightly snappier ease
});

tl.to(image, { scale: 1.05, duration: 0.6 }, 0)
  .to(paragraph, { yPercent: -100, opacity: 0 }, 0)
  .to(button, { 
    yPercent: 0, 
    opacity: 1, 
